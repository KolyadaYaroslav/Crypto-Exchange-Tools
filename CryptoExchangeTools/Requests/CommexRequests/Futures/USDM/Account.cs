...
public class Account
{
    ...
    /// <summary>
    /// Income history only contains data for the last three months. "trandId" is unique in the same incomeType for a user.
    /// </summary>
    /// <param name="symbol"></param>
    /// <param name="incomeType">If incomeType is not sent, all kinds of flow will be returned.</param>
    /// <param name="startTime">Timestamp in ms to get funding from INCLUSIVE. If neither startTime nor endTime is sent, the recent 7-day data will be returned.</param>
    /// <param name="endTime">Timestamp in ms to get funding until INCLUSIVE. If neither startTime nor endTime is sent, the recent 7-day data will be returned.</param>
    /// <param name="page"></param>
    /// <param name="limit">Default 100; max 1000.</param>
    /// <param name="recvWindow"></param>
    /// <returns></returns>
    public IncomeHistoryRecord[] GetIncomeHistory(
        string? symbol = null,
        ...
        long recvWindow = -1)
    {
        var request = BuildGetIncomeHistory(symbol, incomeType, startTime, endTime, page, limit, recvWindow);

        return Client.ExecuteRequest<IncomeHistoryRecord[]>(request);
    }
    ...
    /// <param name="recvWindow"></param>
    /// <returns></returns>
    public async Task<IncomeHistoryRecord[]> GetIncomeHistoryAsync(
        string? symbol = null,
        IncomeType? incomeType = null,
        long startTime = -1,
        long endTime = -1,
        ...
        long recvWindow = -1)
    {
        ...
        return await Client.ExecuteRequestAsync<IncomeHistoryRecord[]>(request);
    }

    private static RestRequest BuildGetIncomeHistory(
        string? symbol,
        ...
        long recvWindow)
    {
        var request = new RestRequest("fapi/v1/income");
        
        if(!string.IsNullOrEmpty(symbol))
            request.AddParameter("symbol", symbol);
        
        if(incomeType is not null)
            request.AddParameter("incomeType", incomeType.ToString());
